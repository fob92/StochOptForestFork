...
direct = ''
date = ''
output = "cvar_lognormal_oracle.txt"
...
for N in N_list:
    ...
    for obj_coef in obj_coef_list:
        ...
        cond_sharpe = evaluate_cond_sharpe(results_eval_all[str(N)][str(obj_coef)], R = R)
        with open(output, 'a') as f:
            print("conditional sharpe ratio:", N, file=f)
            for k,v in sorted(cond_sharpe.items(), key = lambda x: x[1].mean()):
                print(k,"sharpe ratio:",np.mean(v),"+-", 2*np.std(v)/np.sqrt(len(v)), file=f)
            print("------------------------", file = f)
        cond_sharpe_all[str(N)][str(obj_coef)] = cond_sharpe

        marginal_sharpe = evaluate_marginal_sharpe(results_eval_all[str(N)][str(obj_coef)], R = R)
        with open(output, 'a') as f:
            print("marginal sharpe ratio:", N, file=f)
            for k,v in sorted(marginal_sharpe.items(), key = lambda x: x[1].mean()):
                print(k,"sharpe ratio:",np.mean(v),"+-", 2*np.std(v)/np.sqrt(len(v)), file=f)
            print("------------------------", file = f)
        marginal_sharpe_all[str(N)][str(obj_coef)] = marginal_sharpe
        ...
        pickle.dump(results_eval_all, open(direct + date +  "results_eval_cvar_lognormal_oracle.pkl", "wb"))
        pickle.dump(risk_all, open(direct + date +  "risk_cvar_lognormal_oracle.pkl", "wb"))
        pickle.dump(feature_split_all, open(direct + date +  "feature_split_cvar_lognormal_oracle.pkl", "wb"))
