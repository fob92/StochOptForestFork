...
def generate_Y(X, cond_mean, cond_std, Ny = None):
    L = len(cond_mean)
    ...
    for l in range(L):
        if Ny > X.shape[0]:
            ...
            Y[:, l] = get_truncated_normal(low = 0, upp = 100, mean = cond_mean[l](X_temp), sd = cond_std[l](X_temp)).rvs()
        else:
            Y[:, l] = get_truncated_normal(low = 0, upp = 100, mean = cond_mean[l](X), sd = cond_std[l](X)).rvs()
...
def compare_forest_one_run_honesty(X, Y, X_est, Y_est, 
    ...
    balancedness_tol = 0.3, bootstrap = False):
...
def compare_adaptive_nonadaptive_one_run(X, Y, X_est, Y_est, 
    ...
    models['knn_100'] = NearestNeighbors(n_neighbors= 100, algorithm='ball_tree').fit(X)
