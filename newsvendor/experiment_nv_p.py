...
for N in N_list:
    ...
    for p in p_list:
        ...
        time1 = time.time()
        results_fit = Parallel(n_jobs=n_jobs, verbose = 3)(delayed(compare_adaptive_nonadaptive_one_run)(X_list[run], Y_list[run], X_list[run], Y_list[run], 
        ...
        pickle.dump(risk_all, open(direct + date +  "risk_p.pkl", "wb"))
        pickle.dump(feature_split_all, open(direct + date +  "feature_split_p.pkl", "wb"))
        pickle.dump(results_eval_all, open(direct + date +  "results_eval_p.pkl", "wb"))